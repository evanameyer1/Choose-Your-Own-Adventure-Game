...
def bulletamt():
    ...
    for item_index in range(len(inventory)):
        if inventory[item_index]['name'] == 'gun':
            return inventory[item_index]['bullets']
        else:
            pass
...
def action_choices():
    ...
    if curr_action == 'play':
        broke()
        time.sleep(1)
        print("you have " + str(moneyamt()) + " dollars")
        ...
        if checkitem('hat') == True:
            ...
            changemoney(5)
            time.sleep(1)
            print("It must be your lucky hat, good for you.")
            action_pick()
        else:
            blackjack()
    if curr_action == 'run':
    ...
    if curr_action == 'fight':
        ...
        if checkitem('gun') == False and checkitem('knife') == False:
            ...
            changemoney(10)
            time.sleep(2)
            print("You find a solid 10 dollars off of their bodies and leave them for the ravens in the middle of the desert.")
            deadwood()
            ...
            print("You manage to wound three of the 6 and as they fail to land a single hit on you, you somehow manage to get the rest to run away.")
            time.sleep(0.5)
...
def broke():
    if checkmoney(0) == False and (checkitem('tools') == False or checkitem('glass') == False):
...
def location_choices():
    ...
    if curr_loc == 'the dusty road':
        ...
        if checkitem('tools') == True and checkitem('glass pane') == True:
            ...
            time.sleep(3)
            print("It took a while but after a bit you fixed the machine enough to travel again!")
            time.sleep(4)
        ...
        if checkitem('water bottle') != True:
            ...
            print("You might not have enough water to survive the walk and you dont want to die alone out in the desert in a year you dont even know. It'd probably be safer to at least get some water first")
...
def gold_rush_town():
    ...
    if town_first_time:
        print("You ask someone for the year and although they look at you like you're crazy, they politely answer that its currently June 17th of 1873.")
...
def blackjack():
    ...
    def logic():
        ...
        if sum(hand) > 21:
            ...
            broke()
            action_pick()
        elif sum(hand) < 21 and sum(dealer) > 21:
            ...
            while sum(dealer) < 17:
                dealeract()
        elif sum(dealer) >= 17 and sum(hand) > sum(dealer):
            print("\nLucky, you win, you got 5 dollars")
            changemoney(5)
            poss_actions = ['play', 'leave']
            action_pick()
            ...
            poss_actions = ['play', 'leave']
            action_pick()
        elif chose == 'hit' and sum(hand) == 21 and sum(dealer) != 21:
            print("\nLucky, you win, you got 5 dollars")
            changemoney(5)
            poss_actions = ['play', 'leave']
            action_pick()
            ...
            poss_actions = ['play', 'leave']
            action_pick()
        elif sum(hand) == sum(dealer) and chose == 'stand':
            print("\nYou tied, nothing really happens")
            poss_actions = ['play', 'leave']
            action_pick()
    logic()    
...
def shadydudes():
    ...
    time.sleep(0.5)
    print("Terrible idea to be honest.")
    time.sleep(0.5)
    print("\nAs you walk up they stop their conversation and eye you suspiciously.")
    print("Two other dudes walk up behind you and you now seem to be surrounded.")
...
def badblackjack():
    ...
    def logic():
        global poss_actions
        if sum(hand) > 21:
            print("\nUnlucky, you lost, you lose 10 dollars")
            changemoney(-10)
...
print("You've landed in some deserty area, nothing around you but a path that leads off into the distance. It seems like you've landed yourself in the Old West")
#time.sleep(3)
...
print("You do see some items in the machine, that may help you on your journey. You think to yourself and decide you can only take 4 things with you for this jourey.\n")
#time.sleep(3)
...
def item_print(list):
    if list['items'] == 'Gun':
        ...
        return('You take your trusty Glock 43, and find the clip still has ' + str(bullets) + ' bullets left')
    if list['items'] == 'Money':
        money = random.randrange(5,20)
        inventory.append({'name': 'money', 'money' : money})
        return('You find ' + str(money) + ' dollars')
    if list['items'] == 'Water':
        inventory.append({'name': 'water bottle'})
        return('Your water bottle is completely full')
    if list['items'] == 'Tools':
        inventory.append({'name': 'tools'})
        return('These tools that might be useful to fix something')
    if list['items'] == 'Lucky hat':
        inventory.append({'name': 'hat'})
        return('Your trusty Lucky hat has never failed you before')
    if list['items'] == 'Tobacco':
        inventory.append({'name': 'tobacco'})
        return('You find a container with some tobacco, you wonder why you have something like this')
    if list['items'] == 'Knife':
        inventory.append({'name': 'knife'})
        return('A knife would help for sure')
...
print("You are able to run a safe distance away but you can't do anything as you watch your machine get trampled. \n")
time.sleep(3)
print("You head back to check out the damage. Somehow the battery is completely intact and charging but the body got severely dented and the window was shattered.")
if checkitem('tools') == True:
    ...
    print("Unfortunately, your tools were broken by the bison and you didn't take them beforehand so you're going to need to find some tools to fix the body and some glass for the window\n")
...
print("The sign has two arrows, one says |GOLD RUSH TOWN 20 MI| and the other says |DEADWOOD TOWN 2 MI|")
